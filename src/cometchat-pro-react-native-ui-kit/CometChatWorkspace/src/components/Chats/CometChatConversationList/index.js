...
import { chatComment } from '../../../../../../assets/icons';
...
class CometChatConversationList extends React.Component {
  ...
  noChatsFoundPlaceholder = () => {
    return <EmptyState text={"Check the list of Members and click on \n the message icon to start a conversation."}
                source={chatComment}
                imageStyle={styles.noChatsFoundImageStyle}
                containerStyle={styles.noChatsFoundContainerStyle}
    />
  }
      ...
      .then((user) => {
        ...
        this.ConversationListManager.fetchNextConversation()
          .then((conversationList) => {
            if (conversationList.length === 0) {
              this.decoratorMessage = this.noChatsFoundPlaceholder();
            }
            this.setState({
              conversationList: [
