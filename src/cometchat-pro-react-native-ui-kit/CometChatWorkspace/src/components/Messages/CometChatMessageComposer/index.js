...
export default class CometChatMessageComposer extends React.PureComponent {
  ...
  sendTextMessage = ( onSendMessageButtonPressed ) => {
    try {
  ...
  render() {
    const { onSendMessageButtonPressed } = this.props;
    let disabled = false;
    ...
    let sendBtn = (
      <TouchableOpacity
        ...
        onPress={() => this.sendTextMessage( onSendMessageButtonPressed )}>
        <Image
          style={{ width: 40, height: 40}}
          source={require('../../../resources/icons/send_message.png')}
        />
